...
export class TemplateOperations {
  ...
  constructor(
    ...
    this.templateEngine =
      new TemplateEngine(app);
  }

  private createRuleEngine(): TemplateRuleEngine {
    return new TemplateRuleEngine(
      this.settings.rules,
    );
  }
  ...
  async applyToFile(
    ...
    const ruleEngine =
      this.createRuleEngine();

    const matches =
      ruleEngine.match(file.path);
    ...
    try {
      ...
      for (const resolved of selected) {
        const templatePath =
          ruleEngine.resolveTemplatePath(
...
export function summarizeResults(
  ...
  const applied =
    results.filter((x) => x.applied).length;

  const skipped =
    results.filter((x) => x.skipped).length;
